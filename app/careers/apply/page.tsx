...
export default function ApplyPage() {
    ...
    const fetchVacancy = async () => {
      try {
        ...
        const found = data.find(
          (v: Vacancy) => v.id === vacancyId || v._id === vacancyId,
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.fullName || !formData.email) {
      alert(ap?.validationNameEmail || "Please provide your name and email");
      return;
    }

    if (!formData.resume) {
      alert(ap?.validationResume || "Please upload your resume");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          error.error || ap?.errorMessage || "Failed to submit application.",
        );
      }
    } catch (error) {
      console.error("Submit error:", error);
      alert(
        ap?.errorMessage || "Failed to submit application. Please try again.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        alert(ap?.fileSizeError || "File size cannot exceed 5MB");
        return;
      }
  ...
  const experienceOptions = [
    { value: "", label: ap?.selectExperience || "Select" },
    { value: "0-1", label: ap?.experience0_1 || "0-1 Years" },
    { value: "1-3", label: ap?.experience1_3 || "1-3 Years" },
    { value: "3-5", label: ap?.experience3_5 || "3-5 Years" },
    { value: "5-8", label: ap?.experience5_8 || "5-8 Years" },
    { value: "8+", label: ap?.experience8plus || "8+ Years" },
  ];
  ...
  return (
    <main className="relative min-h-screen bg-[#0a0a0f] py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10 mt-20">
        ...
        >
          {ap?.backToCareers || "← Back to Careers"}
        </Link>
        ...
        >
          ...
          <div>
            <h2 className="text-xl font-semibold text-white mb-4 border-b border-white/10 pb-2">
              {ap?.personalInfo || "Personal Information"}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.fullName || "Full Name"}{" "}
                ...
                <input
                  ...
                  placeholder={ap?.placeholderName || "John Doe"}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.email || "Email"} <span className="text-red-500">*</span>
                </label>
                <input
                  type="email"
                  required
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={ap?.placeholderEmail || "john@example.com"}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.phoneNumber || "Phone Number"}
                </label>
                <input
                  type="tel"
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={ap?.placeholderPhone || "+880 1234 567890"}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.experience || "Experience (Years)"}
                </label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.currentCompany || "Current Company"}
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={ap?.placeholderCompany || "Company Name"}
                />
              </div>
            </div>
          </div>
          ...
          <div>
            <h2 className="text-xl font-semibold text-white mb-4 border-b border-white/10 pb-2">
              {ap?.profileLinks || "Profile Links"}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                <label className="block text-sm font-medium text-white/80 mb-1">
                  {ap?.portfolio || "Portfolio/Website"}
                </label>
                <input
                  type="url"
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={
                    ap?.placeholderPortfolio || "https://yourwebsite.com"
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={
                    ap?.placeholderGithub || "https://github.com/username"
                  }
                />
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="text-xs text-white/40 mt-1">
                  {ap?.acceptFormats || "PDF, DOC, DOCX (Max 5MB)"}
                </p>
                {formData.resume && (
                  <p className="text-xs text-green-400 mt-1">
                    ✓ {formData.resume.name} {ap?.fileUploaded || "uploaded"}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 bg-black/50 border border-white/20 rounded-lg text-white focus:border-[#6c5ce7] focus:outline-none"
                  placeholder={
                    ap?.tellAboutYourself ||
                    "Tell us about your experience, skills, and why you are the right fit for this position..."
                  }
                />
              </div>
            </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {submitting
                ? ap?.submitting || "Submitting..."
                : ap?.submit || "Submit Application →"}
            </button>
          </div>
        </motion.form>
      </div>
