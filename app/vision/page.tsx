...
// ✅ Particle class with proper types
class Particle {
  x: number;
  y: number;
  vx: number;
...
class Wire {
  ...
  draw(ctx: CanvasRenderingContext2D, time: number) {
    ...
    for (let i = 1; i < this.points.length; i++) {
      const cp1x =
        this.points[i - 1].x + (this.points[i].x - this.points[i - 1].x) * 0.3;
      const cp1y =
        this.points[i - 1].y + (this.points[i].y - this.points[i - 1].y) * 0.3;
      const cp2x =
        this.points[i].x - (this.points[i].x - this.points[i - 1].x) * 0.3;
      const cp2y =
        this.points[i].y - (this.points[i].y - this.points[i - 1].y) * 0.3;
      ctx.bezierCurveTo(
        cp1x,
        cp1y,
        cp2x,
    ...
    gradient.addColorStop(
      0,
      `rgba(108, 92, 231, ${0.1 + Math.sin(time * 0.001 + this.phase) * 0.05})`,
    );
    gradient.addColorStop(
      1,
      `rgba(0, 206, 201, ${0.1 + Math.cos(time * 0.001 + this.phase) * 0.05})`,
    );
...
export default function VisionPage() {
  ...
  useEffect(() => {
    ...
    const initWires = () => {
      wires = [];
      const wireCount = Math.min(
        15,
        Math.floor((canvas.width * canvas.height) / 50000),
      );
      for (let i = 0; i < wireCount; i++) {
        wires.push(new Wire(canvas.width, canvas.height));
      }
    };
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < maxDistance) {
            ...
            const opacityVal = 0.12 * (1 - distance / maxDistance);

            ctx.strokeStyle = `rgba(108, 92, 231, ${opacityVal})`;
            ctx.lineWidth = 0.6;
            ctx.stroke();
          }
        }
    ...
    const animate = () => {
      ...
      particles.forEach((particle) => {
        ...
        particle.update(mousePosition, canvas.width, canvas.height);
      });
        ...
        >
          ...
          >
            ...
            >
              ...
              shouldn&apos;t be a luxury. Our mission is to make{" "}
              <span className="text-[#00cec9] font-semibold">
                affordable, modern web solutions
              </span>{" "}
            ...
            <div className="relative z-10 text-center">
              ...
              <p className="text-white/70 text-xl md:text-2xl leading-relaxed max-w-4xl mx-auto">
                ...
                <span className="text-white font-bold">
                  {" "}
                  every dreamer, entrepreneur, and small business
                </span>
                deserves a modern, fast, and effective digital platform at an{" "}
            ...
            <div className="bg-gradient-to-r from-[#6c5ce7]/10 via-transparent to-[#00cec9]/10 border border-white/10 rounded-3xl p-10 md:p-14">
              <div className="text-center mb-10">
                ...
                >
                  <TrendingUp
                    size={32}
                    className="text-[#00cec9] mx-auto mb-4"
                  />
                </motion.div>
                <h2 className="text-3xl md:text-4xl font-bold mb-4">
                  Why Grow Business Solutions?
                </h2>
                <p className="text-white/50 text-lg max-w-2xl mx-auto">
                  We don&apos;t just write code — we build digital tools that
